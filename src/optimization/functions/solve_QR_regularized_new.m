function A_opt = solve_QR_regularized_new(E, X, lambda)
   % ROBUST SOLVER: Truncated SVD Pseudoinverse
    ...
    %fprintf('  [Init] Solving using Truncated SVD...\n');
    ...
    % We keep only singular values that are significant relative to the max
    % Standard "pinv" tolerance is often too loose for metabolic matrices
    max_s = max(s_vals);
