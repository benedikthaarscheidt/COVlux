    ...
    for attempt = 1:maxSeedAttempts
        ...
        for k = 1:numCand
            ...
            if acceptType(k) == 1
                ...
                allX{i}        = v_full;
                covered(supp0) = true;
                fprintf('   → accept |supp|=%d | Sv(full)=%.2e | Sv(supp)=%.2e | max off |v|=%.2e', ...
                        numel(supp0), res_full_acc(k), res_supp_acc(k), maxOff_acc(k));
                if idxOff_acc(k)>0
                    fprintf(' @ %d (%s)\n', idxOff_acc(k), rxnNames{idxOff_acc(k)});
                else
                    fprintf('\n');
                end
                found = true; break;
            elseif acceptType(k) == 2
                supp_fix = supp_accept{k};
                v_fix    = v_accept{k};
                allEFMs{i}  = supp_fix(:);
                allX{i}     = v_fix(:);
                covered(supp_fix) = true;
                fprintf('   → accept (single-drop) |supp|=%d | Sv(full)=%.2e | Sv(supp)=%.2e | max off |v|=%.2e', ...
                        numel(supp_fix), res_full_acc(k), res_supp_acc(k), maxOff_acc(k));
                if idxOff_acc(k)>0
                    fprintf(' @ %d (%s)\n', idxOff_acc(k), rxnNames{idxOff_acc(k)});
                else
                    fprintf('\n');
