function enumerate_efms_for_media(model_ir, current_media_name, minimal_substrates, nThreads)
    ...
    if numEFMs == 0
        warning('No EFMs accepted for %s; nothing to save.', current_media_name);
    else
        ...
        % --- DYNAMIC FILENAMES ---
        mat_filename = sprintf('efms_matrix_iML1515_%s.mat', current_media_name);
        csv_filename = sprintf('efms_matrix_iML1515_%s.csv', current_media_name);
        ...
        writetable(EFM_table, csv_filename, 'WriteRowNames', true);
    
        fprintf('Saved %s EFM matrix: (%d x %d) = [residual; %d reactions] x %d EFMs\n', ...
                upper(current_media_name), size(EFM_matrix,1), size(EFM_matrix,2), n, numEFMs);
    end
end 
