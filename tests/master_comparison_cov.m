...
% 6. REPORTING: Generates full CSV and a Summary Table.
% 7. VISUALIZATION: All 10 Figures.
...
% AUTO-DETECT LATEST RUN (or manually set specific run name here)
...
% 3. Covariance Input Directory (MRAS Outputs)
...
% Fallback if MRAS_outputs missing
...
fprintf('\n%-28s | %-5s | %-5s | %-8s | %-6s | %-8s | %-6s | %-5s | %-6s | %-5s | %-5s | %-5s | %-5s | %-8s | %-8s | %-5s | %-5s | %-6s | %-30s\n', ...
    'Cluster (Method)', 'Keep', 'Lost', 'Bio', 'LCC%', 'AlgConn', 'FVA%', 'Block', 'FP', 'PhPP', 'AA%', 'MTA%', 'DeadE', 'Var(X)', 'CorrM', 'Coh.', 'nCC(L)', 'Sz(L)%', 'Most_Pruned_Cat');
fprintf('%s\n', repmat('-', 1, 240));
for k = 1:length(files)
    ...
    exprFile = fullfile(expressionDir, 'mapped_for_benchmarking', [expr_short_name, '_mapped_to_model.csv']);
    ...
    if exist(exprFile, 'file')
        ...
        valid_genes_struct.gene = file_genes(:);  % These are b-numbers
    ...
    for m_idx = 1:3
        ...
        if num_lost > 0
            % Create subgraph of ONLY lost reactions and their metabolites
            S_lost = model.S(:, lost_indices);
            
            % Find metabolites connected to lost reactions
            ...
            if ~isempty(S_lost) && size(S_lost, 2) > 0
                ...
                % Handle case with no edges (all reactions disconnected)
                if nnz(Adj_lost) == 0
                    num_lost_components = n_rxns_lost;  % Each reaction is its own component
                    largest_lost_fraction = 100 / n_rxns_lost;  % Each component is 1/n
                else
                ...
                % All lost reactions are disconnected
...
function [RH_indices, RL_indices] = getExpressionSets(model, geneExpression)
    ...
    fprintf("Lenght of RH indices: %d \n",length(RH_indices));
    RL_indices = RL_tmp(:);
end