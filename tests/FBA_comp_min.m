...
for k = 1:length(files)
    ...
    saveas(f_comp, fullfile(plotDir, sprintf('%s_Method_Comparison_minmedia.png', cluster_full_name)));
    close(f_comp);
    
    % Save CSV Data
    out_table = array2table(method_results_perc, 'VariableNames', method_names, 'RowNames', target_rxns_mod);
    writetable(out_table, fullfile(plotDir, sprintf('%s_Method_Comparison_minmedia.csv', cluster_full_name)), 'WriteRowNames', true);
end
