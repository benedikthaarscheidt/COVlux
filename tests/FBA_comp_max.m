...
lb_base(uptake_idx) = 0; ub_base(uptake_idx) = 1000; 



glc_idx = find(strcmpi(rxnNames, 'ex_glc__d_e_b') | strcmpi(rxnNames, 'ex_glc_e_b'));
if ~isempty(glc_idx), ub_base(glc_idx) = 1000; end 
o2_idx = find(strcmpi(rxnNames, 'ex_o2_e_b'));
if ~isempty(o2_idx), ub_base(o2_idx) = 1000; end
...
for k = 1:length(files)
    ...
    saveas(f_comp, fullfile(plotDir, sprintf('%s_Method_Comparison_maxmedia.png', cluster_full_name)));
    close(f_comp);
    
    % Save CSV Data
    out_table = array2table(method_results_perc, 'VariableNames', method_names, 'RowNames', target_rxns_mod);
    writetable(out_table, fullfile(plotDir, sprintf('%s_Method_Comparison_maxmedia.csv', cluster_full_name)), 'WriteRowNames', true);
end
